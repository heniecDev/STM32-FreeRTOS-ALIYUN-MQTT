...
#define inf  GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_7)//读红外线传感器输出
#define beep  GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_5)//读蜂鸣器输出

//软件延时函数
void delay_soft(unsigned long u)
{
	unsigned long i,j;
...
///*          	WIFI网络与阿里云配置（配置）			      	   */
///*-------------------------------------------------------------*/
//#define  PRODUCTKEY           "a1Ijm32ZA7N"                                        //产品ID
//#define  DEVICENAME           "LWWC17"                                               //设备名  
//#define  DEVICESECRE          "5ff951cb3e80356b04d841b72e29a2cc"                   //设备秘钥   
//#define  S_TOPIC_NAME         "/sys/a1Ijm32ZA7N/LWWC17/thing/service/property/set"   //需要订阅的主题  
//#define  P_TOPIC_NAME         "/sys/a1Ijm32ZA7N/LWWC17/thing/event/property/post"    //需要发布的主题  


//#define SSID   "nova"              //路由器SSID名称   2.4G   
//#define PASS   "3327995054"                 //路由器密码  ****WIFI6****
...
void INF_Init(void){
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;			    //LED0-->PA.1 端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 	 //上拉输入
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	 //IO口速度为50MHz
  GPIO_Init(GPIOA, &GPIO_InitStructure);			     //初始化GPIOA.1
  GPIO_SetBits(GPIOA,GPIO_Pin_7);					//PA.1 输出低
...
//INF任务，槽型光耦
TaskHandle_t INF_Task_Handler;
void my_inf_task(void *pvParameters);
...
//	IoT_parameter_init();  //初始化OneNET平台MQTT服务器的参数	
	AliIoT_Parameter_Init();	//初始化aliyun平台MQTT服务器的参数	
...
void my_start_task(void *pvParameters)
{
    ...
    xTaskCreate(my_lcd_task, 			"my_lcd_task",        		128, NULL, 3, &LCD_Task_Handler);	
		//创建INF任务，槽型光耦
		xTaskCreate(my_inf_task, 			"my_inf_task",        		128, NULL, 3, &INF_Task_Handler);	
...
void my_dht11_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"Humidity\":%d,\"temperature\":%d},\"version\":\"1.0.0\"}",humidity,temperature);  //构建数据
    MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));                  //添加数据到发送缓冲区
		delay_ms(3 * 1000);//延时10s
	}
} 
...
void my_sun_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		//printf("\"sunlight\":\"%d\",", sun_light);		
		xQueueSend(Message_Queue, &data_of_sensor, portMAX_DELAY);  //向消息队列发送消息体，将光照强度数据放入传感器数据消息队列
		sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"LightLux\":%d,},\"version\":\"1.0.0\"}",sun_light);  //构建数据
    MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));                  //添加数据到发送缓冲区
		delay_ms(3 * 1000);//延时10s
	}
} 
...
/*---------------------------------------------------------------*/
/*函数名：void my_mq2_task(void *pvParameters)                 */
/*功  能：MQ2任务 烟雾传感器（配置）							 */
/*		  1.检测环境烟雾ADC值     							     */
/*		  2.将环境环境烟雾ADC放入传感器数据消息队列       			 */
/*		  3.所有数据添加至缓冲区的任务   			 */
/*参  数：无                          			   				 */
/*返回值：无                                       			     */
/*其  他：服务器连接以及PING心跳包30S发送模式事件发生时执行此任务，*/
...
void my_mq2_task(void *pvParameters)
{
	...
	while(1)
	{
		...
		adcx=Get_Adc_Average(ADC_Channel_1,10);		//烟雾数值  ADC
		adcx2=adcx;	//存储烟雾数值
...
//		temp*=1000;			//电压小数部分
		...
		if(adcx2>4000){
			beepOn();
			led1_on();		
			printf("报警开启，烟雾浓度过高，请检查！");
			printf("报警开启，烟雾浓度过高，请检查！");
			printf("报警开启，烟雾浓度过高，请检查！");
		}
		if(temperature>=30){			//温度适当调小，便于测试
			beepOn();
			led1_on();
			printf("报警开启，温度异常过高，请检查！");
			printf("报警开启，温度异常过高，请检查！");
			printf("报警开启，温度异常过高，请检查！");
		}
		if(sun_light>200){
		...
		sprintf(data_of_sensor, "\"ADC_CH1_VAL\":\"%d\",\"ADC_CH1_VOL\":\"%0.3f\",", adcx2, tem); 
		//printf("\"ADC_CH1_VAL\":\"%d\",\"ADC_CH1_VOL\":\"%0.3f\",", adcx2, tem);
		xQueueSend(Message_Queue, &data_of_sensor, portMAX_DELAY);//向消息队列发送消息体，将 烟雾、电压 数据放入传感器数据消息队列
		
		sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"Smoke\":%d,\"LightVolt\":%f,\"Humidity\":%d,\"temperature\":%d,\"LightLux\":%d,},\"version\":\"1.0.0\"}",adcx2,tem,humidity,temperature,sun_light);  //构建数据
    MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));                  //添加数据到发送缓冲区
		delay_ms(3 * 1000);//延时10s
	}
} 
...
/*---------------------------------------------------------------*/
/*函数名：void my_inf_task(void *pvParameters)                 */
/*功  能：槽型光耦任务 红外传感器（配置）							 */
/*		  1.检测环境温湿度数据      							     */
/*		  2.将环境温湿度数据放入传感器数据消息队列       			 */
/*参  数：无                          			   				 */
...
//		char temp[256];
		...
		if(inf==1){
			//构建消息体
			sprintf(data_of_sensor, "\"room person\":\"%s\"", "有人");  	// 1 有人经过	；0 无人经过
			//printf( "\"room person\":\"%s\"", "有人");
//			sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"light\":有人},\"version\":\"1.0.0\"}");  //构建数据
			led1_on();
			beepOn();
			
			printf("{beep:%d},{led1:开}",beep);
		}
		else
		{
			sprintf(data_of_sensor, "\"room person\":\"%s\"", "无人");  	// 1 有人经过	；0 无人经过
//			sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"light\":无人},\"version\":\"1.0.0\"}");  //构建数据
			led1_off();
			beepOff();
			printf("{beep:%d},{led1:关}",beep);
		}
	
		xQueueSend(Message_Queue, &data_of_sensor, portMAX_DELAY);//向消息队列发送消息体，将有人无人数据放入传感器数据消息队列
//		sprintf(temp,"{\"method\":\"thing.event.property.post\",\"id\":\"203302322\",\"params\":{\"light\":有人},\"version\":\"1.0.0\"}");  //构建数据
//    MQTT_PublishQs0(P_TOPIC_NAME,temp,strlen(temp));                  //添加数据到发送缓冲区
...
/*---------------------------------------------------------------*/
/*函数名：void my_lcd_task(void *pvParameters)               */
/*功  能：LCD显示屏任务 LCD（配置）							 */
/*		  1.显示屏输出各个传感器的数据      							     */
/*		  2.有/无人入室盗窃提示      			 */
/*参  数：无                          			   				 */
...
//	char str2[] =  "无人!"; 
	...
	POINT_COLOR=BLUE;//设置字体为蓝色
	LCD_ShowString(60,90,200,16,16,"ADC_CH1_VAL:");	      
	LCD_ShowString(60,110,200,16,16,"ADC_CH1_VOL:0.000V");	
	LCD_ShowString(60,130,200,16,16,"Temperature:			");
	LCD_ShowString(60,150,200,16,16,"Humidity:				");
	LCD_ShowString(60,170,200,16,16,"SunLight:				");
	LCD_ShowString(60,190,200,16,16,"Room_Person:			");
	...
	while(1)
	{
		...
		adcx=Get_Adc_Average(ADC_Channel_1,10);	//烟雾数值
		LCD_ShowxNum(156,90,adcx,4,16,0);//显示ADC的值

		temp=(float)adcx*(3.3/4096);		//转换
		adcx=temp;
		
		//显示电压值  
		LCD_ShowxNum(156,110,adcx,1,16,0);//电压整数部分
		//printf("ADC_CH1_VOL:%f\r\n",adcx+temp);//串口输出电压
		temp-=adcx;
		temp*=1000;
		
		LCD_ShowxNum(172,110,temp,3,16,0x80);//电压小数部分	
		
		
		//温湿度显示
		dht11_read_data(&temperature, &humidity);//读取温湿度值
		LCD_ShowxNum(172,130,temperature,2,16,0x80);
		LCD_ShowxNum(172,150,humidity,2,16,0x80);
		
		//光照强度显示
		sun_light = get_sunlight_value();
		LCD_ShowxNum(172,170,sun_light,5,16,0x80);
		
		//有人无人显示
		if(inf==1){
			LCD_ShowxNum(172,190,1,1,16,0x80);
		}
		else
		{
			LCD_ShowxNum(172,190,0,1,16,0x80);
...
void data_tx_to_buffer_task(void *pvParameters)
{
	...
	{				
		...
		if (xQueueReceive(Message_Queue, data_buffer + 4 + strlen(data_buffer + 4), 10))
		{
			...
			MQTT_PublishQs0(P_TOPIC_NAME, data_buffer, data_len);//添加数据，发布给服务器
			taskEXIT_CRITICAL();  //退出临界区
		}
		delay_ms(3 * 1000);	  //延时10s
	}
